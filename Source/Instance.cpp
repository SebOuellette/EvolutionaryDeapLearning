...
location(startPoint) {
    ...
    for(int i = 0; i < INPUT_COUNT * OUTPUT_COUNT; i++)   //offset the weights by a small amount
        this->weights[i] += fmod(((float)rand() / 10000), 2) - 1;//(Instance::constrain(fmod(rand(), (float)MUTATE_DIVISOR) - (float)MUTATE_DIVISOR/2.f));  //constrains the weight offset to 1 and -1
...
void Instance::run(Location targetLocation) {
    ...
    offset.setX(this->location.getX() * this->weights[0] + this->location.getX() * this->weights[2]);
    
    offset.setY(this->location.getY() * this->weights[1] + this->location.getY() * this->weights[3]);
